                                    ...
                                    usb_callback_data_t *callback_data __attribute__((unused)))
{
    ...
    {
        os_PutStrFull("enable");
        device = (usb_device_t)event_data;
    }
    if (event == USB_DEVICE_DISCONNECTED_EVENT && device)
    {
        os_PutStrFull("disconnect");
        srl_Close(&srl);
...
int main()
{
    ...
    ti_CloseAll();
    ti_var_t slot = ti_OpenVar(ti_Str1, "r", TI_STRING_TYPE);
    if (!slot)
        return 1;
    ...
    do
    {
        ...
        if (device && !has_srl_device)
        {
            ...
            os_PutStrFull("USB connected, press Enter to send");
            do
                kb_ScanGroup(6);
            ...
            uint8_t bytes_read;
            size_t total_read = 0;
            while ((bytes_read = ti_Read(strbuf, 1, 128, slot)))
            {
            ...
            os_ClrHome();
            os_PutStrFull("Write finished, please    disconnect");
        }
